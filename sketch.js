...
function draw() {
  ...
  if (tela == 11) {
    ...
    noStroke(0, 0, 61)
    text('FIM \n \nDE  \n \nJOGO', centro, 150);
    if (mouseX > 200 && mouseX < 320 && mouseY > 400 && mouseY < 460) {
      stroke(200);
      fill(20);
      rect(200, 400, 120, largura);
      fill(213, 213, 213);
      noStroke(0, 0, 61)
      text('EXTRA', centro + 10, 440);
      if (mouseIsPressed) {
        tela = 12;
      }
    }
  ...
  if (tela == 12) {
    ...
    textSize(corpo);
    text('Constelação Antlia', centro, 60);
    text('Conhecida também como a Máquina Pneumática, só\n  pode ser vista no Hemisfério Sul, é melhor vista em\nabril. Fazendo parte da família de constelações de\n Lacaille.', centro, 350);
    if (subtela == 1) {

      line(60, 200, mouseX, mouseY);
      stroke(126);
    ...
    if (subtela == 2) {
      line(60, 200, 150, 120);
      stroke(126);
      line(150, 120, mouseX, mouseY)
      if (mouseX == 300 && mouseY == 190) {
        subtela = 3
      }
    }
    if (subtela == 3) {
      line(60, 200, 150, 120);
      stroke(126);
      line(150, 120, 300, 190)
      tela++
    }
  }
  if (tela == 13) {
    background(esp);
    text('Mensa (A Montanha da Mesa)', centro, 60);
    text('Mensa é uma constelação do Hemisfério Sul\nmelhor vista em janeiro. É a única constelação que se refere a um local concreto,\n a Montanha da Mesa,', centro, 350);
     var x = [100, 145, 230, 250];
    var y = [160, 210, 190, 100];
    var d = [7, 7,7, 7]
    for (j = 0; j < 4; j++) {
      circle(x[j], y[j], d[j]);
    }
    if (subtela == 3) {
      line(100, 160, mouseX, mouseY);
      stroke(126);
      if (mouseX == 145 && mouseY == 210) {
        subtela = 2
      }
    }
    if (subtela == 2) {
      line(100, 160, 145, 210);
      stroke(126);
      line(145, 210, mouseX, mouseY)
      if (mouseX == 230 && mouseY == 190) {
        subtela = 1
      }
    }
    if (subtela == 1) {
      line(100, 160, 145, 210);
      stroke(126);
      line(145, 210, 230, 190)
      line(230, 190, mouseX, mouseY)
      if (mouseX == 250 && mouseY == 100) {
        line(230, 190, 250, 100)
        tela=0
      }
    }
  }
